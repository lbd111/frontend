# mpay PC 端微信收款监控（wx_monitor.py）v1.2

用于在 Windows 10 虚拟机里监听「微信支付」窗口，自动识别收款金额并上报 mpay，完成免签收款。

v1.2 改进：截图后做放大/灰度/二值化预处理，金额提取输出候选列表，优先匹配「收款金额/赞赏金额」附近的 ¥ 金额，降低 0.01 被 OCR 成 0.09 的概率。
...
同一金额默认 300 秒（5 分钟）内只上报一次，避免同一笔收款反复通知。若测试时连续付相同金额，会被跳过，日志会提示剩余秒数。生产环境保持默认；测试时加 `--dedup 10` 或重启脚本清空缓存。
