# mpay PC 端微信收款监控（wx_monitor.py）v1.8

用于在 Windows 10 虚拟机里监听「微信支付」窗口，自动识别收款金额并上报 mpay，完成免签收款。

v1.8 改进：解决 v1.7 中「OCR 未识别到到账时间就跳过不上报」导致的真实收款漏报问题。到账时间改为可选辅助过滤：识别到则用于二次校验；识别不到仍允许上报（pending-amounts 过滤仍是第一道防线）。
...
微信收款通知里通常包含「到账时间 2026-08-07 13:40:09」。v1.8 将其改为可选辅助过滤：
...
若 OCR 未识别到账时间，则按「金额 + 当前时间分桶」去重，避免同一窗口状态被反复上报。
