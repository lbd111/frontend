# mpay PC 端微信收款监控（wx_monitor.py）v1.6

用于在 Windows 10 虚拟机里监听「微信支付」窗口，自动识别收款金额并上报 mpay，完成免签收款。

v1.6 改进：增加到账时间过滤，只上报最近 60 秒内的收款通知，避免微信窗口里残留的旧通知把新订单误匹配成已支付。v1.5 的去重保险（--dedup >=60 秒钳制为 5 秒）和「金额 + 到账时间」去重 key 保留。
...
微信收款通知里通常包含「到账时间 2026-08-07 13:40:09」。脚本只上报**最近 60 秒内**的收款通知；超过 60 秒的旧通知即使还在窗口里，也会被标记为 `[SKIP] 该通知已过期 ... 秒，不上报`。

这解决了测试时「旧 ¥0.01 通知把新订单误匹配成已支付」的问题，不需要每次手动清空聊天记录。
