...
window.addEventListener('scroll', () => {    const navbar = document.querySelector('.navbar');
    ...
    if (navbar) {        if (window.scrollY > 50) {            navbar.classList.add('scrolled');
...
function toggleMenu() {    const menu = document.getElementById('navMenu');
    ...
    }}// --- 点击菜单项后自动关闭 ---document.addEventListener('click', (e) => {    if (e.target.classList.contains('nav-link')) {        const menu = document.getElementById('navMenu');        const hamburger = document.getElementById('hamburger');        if (menu && menu.classList.contains('open')) {            menu.classList.remove('open');            hamburger.classList.remove('active');        }    }});// --- 登录弹窗 ---
...
function closeLoginModal() {    const modal = document.getElementById('loginModal');
    ...
    if (modal) {        modal.classList.remove('active');
        ...
        document.body.style.overflow = '';    }}// --- Tab切换 ---
function switchTab(tab) {    const tabs = document.querySelectorAll('.modal-tabs .tab');
    ...
    if (tab === 'login') {        tabs[0].classList.add('active');
        ...
        loginForm.style.display = 'block';        registerForm.style.display = 'none';    } else {        tabs[1].classList.add('active');

        loginForm.style.display = 'none';        registerForm.style.display = 'block';    }}// --- 订单弹窗 ---let currentWizardPrice = 0;
﻿function showOrderModal(name, price, avatar, skills) {
    ...
    if (modal && wizardName && wizardPrice) {
        ...
        if (serviceTypeSelect && skills) {
            var rawSkills = '';
            try { rawSkills = JSON.parse(skills); } catch(e) { rawSkills = []; }
            if (!Array.isArray(rawSkills) || rawSkills.length === 0) {
                rawSkills = [];
            }
            // skillMap: 将Supabase中的原始技能标签映射为下拉框显示文本
            var skillMap = {
                '跑图':'跑图服务','跑图服务':'跑图服务','跑步':'跑图服务',
                '拍照':'拍照陪玩','换情头':'拍照陪玩','拍照陪玩':'拍照陪玩','photo-edit':'拍照陪玩',
                '收集':'收集陪伴','献祭':'收集陪伴','破晓':'收集陪伴','sacrifice':'收集陪伴','收集陪伴':'收集陪伴',
                '聊天':'聊天交友','闲聊':'聊天交友','聊天交友':'聊天交友','树洞':'聊天交友','treehole':'聊天交友',
                '娱乐':'娱乐陪伴','三恋':'娱乐陪伴','送心':'娱乐陪伴','娱乐陪伴':'娱乐陪伴',
                '技术':'技术教学','教学':'技术教学','教跑图':'技术教学','技术教学':'技术教学',
                '普陪':'普通陪玩','普通陪玩':'普通陪玩','normal':'普通陪玩',
                '琴陪':'琴陪','piano':'琴陪',
                '打龙':'打龙服务','驯龙':'打龙服务','dragon-taming':'打龙服务',
                '替身':'替身服务','substitute':'替身服务',
                '打卡':'监督打卡','checkin':'监督打卡','监督':'监督打卡',
                '挂机':'挂机陪','hang':'挂机陪',
                '录歌':'录音陪玩','record':'录音陪玩',
                '盲盒':'盲盒'
            };
...
function calcOrderTotal() {    const hoursInput = document.querySelector('#orderForm input[type=\"number\"]');
    ...
    if (hoursInput && totalPrice) {        const hours = parseInt(hoursInput.value) || 1;        const total = hours * currentWizardPrice;        totalPrice.textContent = '￥' + total.toFixed(2);

    }}// --- 下载APP ---
function downloadApp(platform) {    if (platform === 'android') {    } else if (platform === 'ios') {    } else {        // 显示下载选择        const choice = confirm('您想下载哪个平台的APP？\\n\\n确定 = Android\\n取消 = iOS');        if (choice) {            downloadApp('android');        } else {            downloadApp('ios');        }    }}// --- 通知系统（全局） ---
function showNotification(message, type) {    const toast = document.createElement('div');

    toast.className = 'notification-toast notification-' + type;    toast.textContent = message;    document.body.appendChild(toast);
...
}    // 订单表单    const orderForm = document.getElementById('orderForm');    if (orderForm) {        orderForm.addEventListener('submit', (e) => {            if (e && e.preventDefault) e.preventDefault();            closeOrderModal();            orderForm.reset();        });
}    // 点击弹窗外部关闭    document.querySelectorAll('.modal-overlay').forEach(overlay => {        overlay.addEventListener('click', (e) => {            if (e.target === overlay) {                overlay.classList.remove('active');                document.body.style.overflow = '';            }        });
});


// --- 星星粒子效果 ---    
function createParticles() {        const heroSection = document.querySelector('.hero-section');
 ...
 i++) {            const star = document.createElement('div');

            star.style.cssText = 'position:absolute;width:' + (Math.random() * 4 + 2) + 'px;height:' + (Math.random() * 4 + 2) + 'px;background:white;border-radius:50%;left:' + (Math.random() * 100) + '%;top:' + (Math.random() * 100) + '%;opacity:' + (Math.random() * 0.7 + 0.3) + ';animation:twinkle ' + (Math.random() * 3 + 2) + 's infinite alternate;';            particleContainer.appendChild(star);

        }        heroSection.appendChild(particleContainer);

    }    createParticles();

    // --- 平滑滚动到锚点 ---    document.querySelectorAll('a[href^=\"#\"]').forEach(anchor => {        anchor.addEventListener('click', function(e) {            if (e && e.preventDefault) e.preventDefault();            const target = document.querySelector(this.getAttribute('href'));            if (target) {                target.scrollIntoView({ behavior: 'smooth', block: 'start' });
}        });
...
// --- 当英雄区域可见时启动数字动画 ---    const heroObserver = new IntersectionObserver((entries) => {        entries.forEach(entry => {            if (entry.isIntersecting) {                animateNumbers();            }        });
}, { threshold: 0.5 });
...
function searchWizards(keyword) {        console.log('搜索陪玩师:', keyword);
    ...
    }    // showNotification moved to global scope (see top of file)    // --- 本地存储工具 ---    const Storage = {        get(key) {            try { return JSON.parse(localStorage.getItem(key)); } catch { return null; }        },        set(key, value) {            localStorage.setItem(key, JSON.stringify(value));        },        remove(key) {            localStorage.removeItem(key);        }    };    window.Storage = Storage;    // --- 移动端菜单 ---    const mobileMenuBtn = document.getElementById('mobileMenuBtn');    if (mobileMenuBtn) {        mobileMenuBtn.addEventListener('click', toggleMenu);    }    // --- 用户菜单 ---        
...
function handleLogout(e) {            if (e && e.preventDefault) e.preventDefault();
 ...
 } catch(err) {}            localStorage.removeItem('skyUser');
...
function updateNavUser() {    try {        var user = localStorage.getItem('skyUser');
        ...
        }    } catch (err) {        console.error('[导航更新异常]', err);
...
function loadOrders() {    try {        const userStr = localStorage.getItem('skyUser');
        ...
        const user = JSON.parse(userStr);
                ...
                const { data, error } = await window.supabaseClient            .from('orders')            .select('*')            .eq('user_id', user.id)            .order('created_at', { ascending: false });
...
function createOrder(orderData) {    try {        const userStr = localStorage.getItem('skyUser');
        ...
        const user = JSON.parse(userStr);
                ...
                const { data, error } = await window.supabaseClient            .from('orders')            .insert({                user_id: user.id,                wizard_id: orderData.wizardId,                wizard_name: orderData.wizardName,                hours: orderData.hours,                total_price: orderData.totalPrice,                status: '待支付'            })            .select()            .single();
...
function loadCoupons() {    try {        const userStr = localStorage.getItem('skyUser');
        ...
        const user = JSON.parse(userStr);
                ...
                const { data, error } = await window.supabaseClient            .from('coupons')            .select('*')            .eq('user_id', user.id)            .eq('used', false)            .order('expire_date', { ascending: true });
...
function grantWeeklyCoupon(userId) {    try {        const today = new Date().toISOString().split('T')[0];
        ...
        const { data, error } = await window.supabaseClient            .from('coupons')            .insert({                user_id: userId,                amount: 5.00,                condition: '满20元可用',                expire_date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],                used: false            });
...
function loadFavorites() {    try {        const userStr = localStorage.getItem('skyUser');
        ...
        const user = JSON.parse(userStr);
                ...
                const { data, error } = await window.supabaseClient            .from('favorites')            .select('*')            .eq('user_id', user.id)            .order('created_at', { ascending: false });
...
function addFavorite(wizardId, wizardName) {    try {        const userStr = localStorage.getItem('skyUser');
        ...
        const user = JSON.parse(userStr);
                ...
                const { error } = await window.supabaseClient            .from('favorites')            .insert({                user_id: user.id,                wizard_id: wizardId,                wizard_name: wizardName            });
...
function removeFavorite(favoriteId) {    try {        const { error } = await window.supabaseClient            .from('favorites')            .delete()            .eq('id', favoriteId);
...
// ==================== 账户余额 ====================async 
function updateBalance(amount) {    try {        const userStr = localStorage.getItem('skyUser');

        if (!userStr) return false;

        const user = JSON.parse(userStr);

                const { data: profiles, error } = await window.supabaseClient            .from('profiles')            .select('balance')            .eq('id', user.id)            .limit(1);

        const profile = profiles && profiles.length > 0 ? profiles[0] : null;

        const newBalance = (parseFloat(profile.balance) + parseFloat(amount)).toFixed(2);

                const { error: updateError } = await window.supabaseClient            .from('profiles')            .update({ balance: newBalance })            .eq('id', user.id);

                if (updateError) return false;

                // 同步到 localStorage        user.balance = newBalance;        localStorage.setItem('skyUser', JSON.stringify(user));                return true;    } catch (err) {        console.error('更新余额失败:', err);        return false;    }}// 暴露全局函数window.loadOrders = loadOrders;window.createOrder = createOrder;window.loadCoupons = loadCoupons;window.grantWeeklyCoupon = grantWeeklyCoupon;window.loadFavorites = loadFavorites;window.addFavorite = addFavorite;window.removeFavorite = removeFavorite;window.updateBalance = updateBalance;
// ==================== 全局同步余额 ====================async 
function syncBalanceFromDB() {    try {        const userStr = localStorage.getItem('skyUser');
        ...
        // 更新 localStorage        user.balance = balance;        localStorage.setItem('skyUser', JSON.stringify(user));        // 更新充值中心余额显示        const rechargeEl = document.getElementById('balanceValue');        if (rechargeEl) {            rechargeEl.textContent = balance.toFixed(2);        }        // 更新个人中心余额显示        const statCards = document.querySelectorAll('.stat-card');        if (statCards && statCards[3]) {            const v = statCards[3].querySelector('.stat-value');            if (v) v.textContent = '\uffe5' + balance.toFixed(2);        }        return balance;    } catch (err) {        console.error('同步余额失败:', err);        return null;    }}window.syncBalanceFromDB = syncBalanceFromDB;
